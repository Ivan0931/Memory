...
const cardName = ['C', 'D', 'H', 'S'],
  ...
  img = `<div class=\"cards\" data-tid=\"Card\"><img src=\"img/back.png\" class=\"card-back\"></div>`
let acc = 0, //accumulator of clicks on the card field
...
function getShuffledCards(cards) {
  const newArr = shuffleCards(cards).slice(0, 9)
...
function createNewCardsLayout(cards = getShuffledCards(cardsArray())) {
...
function gameIsDone(openedCards) {
  if (openedCards == 18) {
    ...
    const summary = document.getElementById('summary-points')
...
function eventOnClick(fieldCards) {
  fieldCards.addEventListener('click', event => {
    ...
    // click on a card which was flipped and has a pair
    ...
    if (currentCard.hasAttribute('data-tid') && currentCard.dataset.tid == 'Card') {
      ...
      if (acc <= 2) {
        ...
        //current index of first flipped card
        const i = accCards.length - 1
        ...
        if (accCards.length % 2 == 0 && accCards[i].cardName != accCards[i - 1].cardName) {
          acc = 0
